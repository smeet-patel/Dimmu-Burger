<?php
	// need user name, pass word  and database name.
		...
		if (isset($_POST['newburger']))
		{
			...
			$burgerstate = mysqli_escape_string($db,"new");
			$queryMax = mysqli_query($db, "SELECT MAX(burgername) AS ordernumber FROM recipes");
			$row = mysqli_fetch_array($queryMax);
			$name = mysqli_escape_string($db,$row['ordernumber']);
			$sql= "SELECT COUNT(ordernumber) from orders;";
			$orderMax = mysqli_query($db, "select max(ordernumber)+1 AS ordernumber from orders;");
			$zero="0";
			if (($sql)==$zero) { 
				$nameone = mysqli_escape_string($db,"1");
				echo $sql." ".$zero; 
			}
			if(($sql)>$zero)  {
					$rowone = mysqli_fetch_array($orderMax);
					$nameone = mysqli_escape_string($db,$rowone['neworder']);
				}

			// echo $nameone." ".$Burger;
			// $burgername = mysql_real_escape_string($name);

			$db->query("INSERT INTO  orders(ordernumber, subordernumber, burgername, burgerstate)
			VALUES ('$nameone','$Burger','$name','new')");
	...
	<body>
		...
		<div class="sticky">
			...
			{if($db == true){
					...
					echo "<td class='totalrow'>Burger # " . $row['ordernumber'] . "</td>";
					echo "</tr>";
				}
						...
						<table id="custom noHov">
							...
							<tr>
								<td>
									...
									<div class="cen"><input type="submit" class="btnSub" name="newburger" align="center" value="Proceed with order"
										style=" color: red;font-size: calc(12px + 1.5vw);margin: 5px;padding: 0.2em;cursor: pointer;border-color: 
										red;background-color:transparent;border-radius: 5px;" onMouseOver="this.style.color='white'"
   										onMouseOut="this.style.color='red'"onclick="finalsubmit()" /></div>
